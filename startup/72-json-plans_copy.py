...
def diff_scan_to_loop(angle, diff_params, ic_init, do_y_offset = True,tracking_file = None):
...
def run_diff_json(path_to_json,tracking_file = None):
    ...
    th_init = th_motor.position
    y_init = fly_motors[1].position
    do_y_offset = diff_params["do_y_offset"]
    diff_params["th_init"] = th_init
    diff_params["y_init"] = y_init
    ...
    for n,angle in enumerate(tqdm.tqdm(angles,desc = 'diff Scan')):
        ...
        if not angle in np.array(diff_params["remove_angles"]):
            ...
            yield from diff_scan_to_loop(angle,diff_params,ic_0,do_y_offset = do_y_offset,tracking_file = tracking_file)
