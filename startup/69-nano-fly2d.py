...
def scan_and_fly_2d(detectors, xcenter, xrange, xnum, ystart, ystop, ynum, dwell, *,
    ...
    md['scan']['scan_input'] = [float(0),float(xrange),xnum,ystart,ystop,ynum,dwell]
    md['scan']['sample_name'] = ''
    ...
    def plan():
        ...
        def zebra_kickoff():
            ...
            yield from kickoff(flying_zebra_2d,
                                ystart=start_zebra, ystop=stop_zebra, num=num_total, dwell=dwell, delta=np.abs(delta),
                                wait=True)
...
class SRXFlyer2D(Device):
    ...
    def kickoff(self, *, ystart, ystop, num, dwell, delta = 0.1, pulse_dur = 0.3):
...
def pt_fly2dcont(dets, motor1, scan_start1, scan_end1, num1, motor2, scan_start2, scan_end2, num2, exposure_time, pos_return = True, **kwargs):
    ...
    try:
        center1, range1 = m1_pos + (scan_start1+scan_end1)/2, np.abs(scan_end1-scan_start1)
...
def test_2dfly():
        ...
        def t2dfly():
            ...
            yield from pt_fg.on()
            yield from bps.sleep(10)
            #yield from scan_and_fly_2d([eiger2],0, 1, 50, 0, 0.2, 10, 0.01,flying_zebra_2d=nano_flying_zebra_2d,xmotor=pt_tomo.ssx,ymotor=pt_tomo.ssy,verbose=True)
            yield from pt_fg.off()
        RE(t2dfly())
