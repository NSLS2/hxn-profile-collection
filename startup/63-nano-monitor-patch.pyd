...
async def ts_monitor(self, msg):
    ...
    # NOTE: The following code was changed to emit events instead of event page.
    #   The events are invalid (events may contain varying number of points),
    #   but sufficient for monitoring. The stream can not be loaded as a table.
    #   Change it back to event pages once Databroker is updated.
    #   ##! - commented lines, which need to be uncommented.
    def emit_event_page(*args, **kwargs):
        ...
        try:
            ...
            self.emit_sync(DocumentNames.event, event_page)
...
def ts_monitor_during_wrapper_disabled(plan, signals):
    """
    Use instead of 'ts_monitor_during_wrapper' if no monitoring is needed or possible.
    """
    return (yield from plan)


ts_monitor_during_decorator_disabled = make_decorator(ts_monitor_during_wrapper_disabled)
